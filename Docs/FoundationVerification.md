...
| EditMode suite | Passed — 51/51 | `TestResults/editmode-side-z-rotation.xml` |
| PlayMode suite | Passed — 13/13 | `TestResults/playmode-side-z-rotation.xml` |
| Windows x64 development build | Passed | `Build/Windows/WhatLightRemains.exe` (201,800,626 total build bytes) and `TestResults/windows-build-side-z-rotation.log` |
...
- all four Z-axis orientations for side placement, aligned side doorways, side-to-ceiling traversal, sealed incompatible side contacts, ceiling targeting, floor rejection, upright sealed stacking, side-down traversable stacking, and ceiling-down sealed stacking;
...
6. While targeting a side wall, rotate through all four Z-axis orientations and confirm the ghost remains visible. A candidate ceiling facing the source wall should preview a traversable ceiling-to-side passage; floor-facing or upside-down side pairings should remain visibly sealed. Return to a floor-aligned orientation, place the room, and walk through the centered side doorway.
...
- A floor cannot be targeted directly. A ceiling-to-side connection is traversable whether initiated from a ceiling or a neighboring side wall; incompatible floor-facing and misaligned side contacts are intentionally sealed.
